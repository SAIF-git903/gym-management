...
export default function Dashboard() {
  ...
  if (error) {
    return <div className="rounded-lg bg-red-50 px-4 py-3 text-red-800 dark:bg-red-950/40 dark:text-red-200">{error}</div>
  }
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        {rev?.byPlan?.length > 0 && (
          <ul className="mt-4 space-y-2 text-sm">
            {rev.byPlan.map((row) => (
              <li key={row.plan} className="flex justify-between border-b border-zinc-100 py-2 dark:border-zinc-800">
